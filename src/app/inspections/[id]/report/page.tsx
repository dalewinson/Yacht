...
function V2Report({ data, logoUrl }: { data: any; logoUrl: string | null }) {
  ...
  return (
    <div className="bg-[var(--color-background-tertiary)] min-h-full">
      <div className="no-print flex items-center justify-between max-w-[800px] mx-auto px-4 pt-4">
        <Link href="/inspections" className="text-[12px] text-[#185FA5] hover:underline inline-flex items-center gap-1"><i className="ti ti-arrow-left text-[13px]" /> Back</Link>
        <PrintButton />
      </div>
      <div id="print-root" className="max-w-[800px] mx-auto bg-white p-8 my-4 text-[12px] text-[#1c1917] leading-relaxed">
        <div className="flex items-start justify-between border-b-2 border-[#185FA5] pb-3 mb-4">
          <div>
            <div className="text-[11px] uppercase tracking-widest text-[#185FA5] font-semibold">Fairwinds</div>
            <h1 className="text-[18px] font-bold mt-0.5">Maintenance Inspection</h1>
            {logoUrl ? <img src={logoUrl} alt={data.vessel_name} className="h-9 object-contain mt-1" /> : <div className="text-[13px] font-medium mt-0.5">{data.vessel_name}</div>}
          </div>
          <div className="text-right text-[11px] text-[#57534e]">
            <div><span className="font-semibold">{data.month} {data.year}</span></div>
            <div>Date: {fmtDate(data.date)}</div>
        ...
        <div className="mb-5 border border-[#E4C38A] rounded-[6px] overflow-hidden">
          <div className="bg-[#FAEEDA] px-3 py-1.5 text-[12px] font-semibold text-[#854F0B]">Items needing attention {flagged.length > 0 && `(${flagged.length})`}</div>
          <div className="px-3 py-2">
            {flagged.length === 0 ? <div className="text-[12px] text-[#3B6D11]">All items checked OK.</div> : (
              <ul className="space-y-1">{flagged.map((f, i) => <li key={i} className="text-[12px]"><span className="font-medium">{f.where} — {f.item}</span>{f.note && <span className="text-[#57534e]">: {f.note}</span>}</li>)}</ul>
            )}
          </div>
        </div>

        {snapshot.map(area => (
          <div key={area.area} className="mb-5">
            <div className="text-[14px] font-bold text-[#185FA5] border-b border-[#d6d3d1] pb-1 mb-2">{area.area}</div>
            {area.equipment.map(eq => {
              const ea = answers[eq.id]
              return (
                <div key={eq.id} className="mb-3">
                  <div className="text-[12px] font-semibold">{eq.name} <span className="text-[10px] font-normal text-[#78716c]">{eq.category}{eq.hoursTracked && ea?.hours ? ` · ${ea.hours} hrs` : ''}</span></div>
                      ...
                      {eq.items.map(it => {
                        ...
                        return (
                          <tr key={it.id} className="border-t border-[#f0eeec]">
                            ...
                            <td className="py-1 px-2 w-[8%] text-center">{mark}</td>
                            <td className="py-1 px-2 w-[16%] text-[#44403c]">{it.field_type !== 'ok' ? (a?.value ?? '') : ''}</td>
                            <td className="py-1 pl-2 text-[#57534e]">{a?.notes ?? ''}{it.scheduled && a?.done ? ' · serviced' : ''}</td>
                          </tr>
                        )
                      })}
                    </tbody>
        ...
        <div className="text-[10px] text-[#a8a29e] mt-6 pt-2 border-t border-[#e7e5e4]">Generated by Fairwinds · {data.vessel_name} · {data.month} {data.year}</div>
      </div>
    </div>
  )
}
